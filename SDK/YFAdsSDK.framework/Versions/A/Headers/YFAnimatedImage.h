...
//
@interface YFAnimatedImage : NSObject
...
// Pass either a `UIImage` or an `YFAnimatedImage` and get back its size
+ (CGSize)sizeForImage:(id)image;

// On success, the initializers return an `YFAnimatedImage` with all fields initialized, on failure they return `nil` and an error will be logged.
...
@interface YFAnimatedImage (Logging)
