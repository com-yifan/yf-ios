...
/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 ...
 - `YFDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<YFResultSet>` - Represents the results of executing a query on an `YFDatabase`.
 - `<YFDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<YFDatabasePool>` - A pool of `YFDatabase` objects.
 - `<YFStatement>` - A wrapper for `sqlite_stmt`.
 ...
 @warning Do not instantiate a single `YFDatabase` object and use it across multiple threads. Instead, use `<YFDatabaseQueue>`.
...
/** Create a `YFDatabase` object.
 
 An `YFDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `YFDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `YFDatabase` connection is closed.
 ...
 Or, in iOS, you might open a database in the app's `Documents` directory:
    ...
    YFDatabase *db     = [YFDatabase databaseWithPath:dbPath];
 ...
 @return `YFDatabase` object if successful; `nil` if failure.
...
/** Create a `YFDatabase` object.
 ...
 2. `nil`.  An in-memory database is created.  This database will be destroyed with the `YFDatabase` connection is closed.
...
/** Initialize a `YFDatabase` object.
...
/** Initialize a `YFDatabase` object.
 ...
 An `YFDatabase` is created with a local file URL to a SQLite database file.  This path can be one of these three:
...
/** Execute select statement
 ...
 Executing queries returns an `<YFResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[YFResultSet next]>`) from one record to the other.
 ...
 @return A `<YFResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see YFResultSet
 @see [`FMResultSet next`](<[YFResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
...
/** Execute select statement
 ...
 `func executeQuery(sql: String, values: [Any]?) throws  -> YFResultSet!`
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using YFDatabase::storeableDateFormat.
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using FMDB you will not need to interact directly with `YFStatement`, but rather with `<YFDatabase>` and `<YFResultSet>` only.
